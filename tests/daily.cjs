...
(async()=>{
 ...
 who={...who,role:'student'};await ui.open();assert.equal(d.querySelectorAll('#dailyContent .daily-card').length,2);assert.equal($('#dailyContent img'),null);assert.equal(progress.at(-1).totalQuestions,2);assert.deepEqual(progress.at(-1).submittedSlots,[]);
 ...
 $('#dailyAnswer0').value='42';$('#dailyReason0').value='推理过程';await $('form').onsubmit({preventDefault(){},currentTarget:$('form')});assert.equal(answers.length,1);assert.deepEqual(progress.at(-1).submittedSlots,[0]);assert.match($('.daily-status').textContent,/已提交/);assert.equal(h.disabled,false);assert.equal($('.daily-step-btn').disabled,false);h.click();assert.match($('.daily-hints').textContent,/先观察/);$('.daily-step-btn').click();assert.match($('.daily-steps').textContent,/第一步/);assert.doesNotMatch($('.daily-steps').textContent,/第二步/);$('.daily-step-btn').click();assert.equal($('.daily-step-btn').disabled,true);
 assert.equal($('.daily-answer-btn').disabled,false);
 const choice=d.querySelectorAll('input[name="dailyChoice1"]')[1];assert(choice);choice.checked=true;const secondForm=d.querySelectorAll('#dailyContent form')[1];await secondForm.onsubmit({preventDefault(){},currentTarget:secondForm});assert.equal(answers.length,2);assert.equal(answers.find(x=>x.slot===1).answer,'B');assert.deepEqual(progress.at(-1).submittedSlots.sort(),[0,1]);
