...
const cap=profile();cap.economy.checkin.days[today]={points:39};const q=E.question(E.bank[0],rng),r=E.settle(cap,{id:'cap',q,started:Date.now()},q.blanks.map(n=>q.item.parts[n]),{today});assert.equal(r.gain,1);assert.equal(cap.economy.credits,1);assert.equal(cap.economy.checkin.days[today].points,40);
console.log('PASS review: all 47 cards, random 2–6 blanks, locked/future exclusion, recent-first, 4/5 chapter quota, sparse fallback, strict grading, 5-in-row repair/reset, duplicate prevention, 40-R cap');
