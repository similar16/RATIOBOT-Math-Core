...
async function open(date){
 ...
 const old=(result.data||[]).find(x=>x.slot===slot),hints=q.hints?.filter(Boolean)||[],choice=q.answer_type==='choice'&&Array.isArray(q.choices)&&q.choices.length>=2,answerField=choice?`<fieldset class="daily-choice-group"><legend>我的答案</legend>${q.choices.map((x,j)=>`<label><input type="radio" name="dailyChoice${slot}" value="${String.fromCharCode(65+j)}" ${old?.answer===String.fromCharCode(65+j)?'checked':''}> <b>${String.fromCharCode(65+j)}.</b> ${esc(x)}</label>`).join('')}</fieldset>`:`<label for="dailyAnswer${slot}">我的答案</label><input class="daily-fill-answer" id="dailyAnswer${slot}" maxlength="500" value="${esc(old?.answer||'')}" placeholder="填写答案" autocomplete="off">`;return `<article class="daily-card" data-slot="${slot}"><p class="daily-note">${esc(day)} · ${rewardRows.some(x=>x.set_id===r.data.id&&x.slot===slot)?'已获额外 5 R':day===today()?'首次提交答案 +5 R · 提交后解锁提示':'往期练习 · 提交后解锁提示'}</p><h3>挑战 ${slot+1} · ${esc(q.title||'动动脑筋')}</h3><div class="daily-pre">${window.QuestionContent?.content(q.body,q.images)||esc(q.body)}</div>${window.QuestionContent?.hasImageMarkers(q.body)?window.QuestionContent?.images(q.extra_images):window.QuestionContent?.images([...(q.images||[]),...(q.extra_images||[])])||''}<button class="ghost daily-hint-btn" ${hints.length&&old?'':'disabled'}>${!hints.length?'老师未设置提示':old?'给我一点提示':'提交答案后看提示'}</button><div class="daily-hints" aria-live="polite"></div>${q.steps?.length?`<button class="ghost daily-step-btn" ${old?'':'disabled'}>${old?'看关键步骤':'提交答案后看步骤'}</button><div class="daily-steps" aria-live="polite"></div>`:''}<form>${answerField}<label for="dailyReason${slot}">解题过程</label><textarea id="dailyReason${slot}" maxlength="8000" placeholder="可以写算式，也可以写你的思考过程">${esc(old?.reasoning||'')}</textarea><button class="primary" type="submit">${old?'更新作答':'提交作答'}</button><div class="daily-status" role="status">${old?'已提交，可查看提示。':''}</div></form>${q.has_solution?`<button class="ghost daily-answer-btn" ${old?'':'disabled'}>${old?'查看答案与解析':'提交作答后可看答案'}</button><div class="daily-solution" hidden></div>`:''}</article>`;
 }).join('')+'</div>';
 ...
 btn.onclick=()=>{if(shown>=hints.length)return;const p=document.createElement('div');p.className='daily-hint';p.textContent=`提示 ${shown+1}：${hints[shown++]}`;if(window.QuestionContent)p.innerHTML=window.QuestionContent.content(p.textContent,currentSet.questions[slot].images);card.querySelector('.daily-hints').append(p);btn.textContent=shown===hints.length?'提示已全部展开':'再给一点提示';btn.disabled=shown===hints.length;window.QuestionContent?.math(p);};
 const stepBtn=card.querySelector('.daily-step-btn');let stepIndex=0;const steps=currentSet.questions[slot].steps||[];if(stepBtn)stepBtn.onclick=()=>{const p=document.createElement('div');p.className='daily-hint';p.textContent=`步骤 ${stepIndex+1}：${steps[stepIndex++]}`;if(window.QuestionContent)p.innerHTML=window.QuestionContent.content(p.textContent,currentSet.questions[slot].images);card.querySelector('.daily-steps').append(p);window.QuestionContent?.math(p);stepBtn.disabled=stepIndex===steps.length;stepBtn.textContent=stepBtn.disabled?'步骤已全部展开':'看下一步';};
 const answerBtn=card.querySelector('.daily-answer-btn');if(answerBtn)answerBtn.onclick=async()=>{if(key!==identityKey())return;const box=card.querySelector('.daily-solution');answerBtn.disabled=true;box.hidden=false;box.textContent='正在读取答案…';try{const sol=await a.client().from('daily_challenge_solutions').select('answer').eq('set_id',r.data.id).eq('slot',slot).maybeSingle();if(sol.error)throw sol.error;if(seq!==request||key!==identityKey())return;box.textContent=sol.data?.answer||'请先提交本题作答后再查看。';if(window.QuestionContent)box.innerHTML=window.QuestionContent.content(box.textContent,currentSet.questions[slot].images);box.classList.add('daily-pre');window.QuestionContent?.math(box);}catch(err){if(seq===request&&key===identityKey())box.textContent='答案加载失败，请重试。';}finally{answerBtn.disabled=false;}};
 card.querySelector('form').onsubmit=async e=>{e.preventDefault();const button=e.currentTarget.querySelector('button'),message=card.querySelector('.daily-status'),q=currentSet.questions[slot],choice=q.answer_type==='choice';const answer=(choice?card.querySelector(`input[name="dailyChoice${slot}"]:checked`)?.value:(card.querySelector(`#dailyAnswer${slot}`)?.value||'')).trim(),reasoning=card.querySelector(`#dailyReason${slot}`).value.trim();if(!answer)return status(message,choice?'请先选择一个答案。':'请先填写答案。',true);
 if(key!==identityKey()||!studentReady())return status(message,'登录状态已变化，请重新打开页面。',true);
 button.disabled=true;status(message,'正在保存…');try{const saved=await a.client().from('daily_challenge_answers').upsert({set_id:r.data.id,user_id:i.user.id,slot,answer,reasoning,submitted_at:new Date().toISOString()},{onConflict:'set_id,user_id,slot'}).select('slot');if(saved.error)throw saved.error;if(!saved.data?.length)throw Error('保存未成功，请重试');if(seq!==request||key!==identityKey())return;status(message,'✓ 已提交，老师可以查看。');try{const rewards=await refreshRewards();if(seq!==request||key!==identityKey())return;status(message,rewards.some(x=>x.set_id===r.data.id&&x.slot===slot)?'✓ 已提交 · 本题额外 5 R 已到账（仅一次）':'✓ 已提交 · 往期或此前已提交题目不重复发奖。');}catch(e){status(message,'✓ 作答已保存。积分状态读取失败，重新打开页面会自动恢复。',true);}button.textContent='更新作答';if(hints.length&&shown<hints.length){btn.disabled=false;btn.textContent=shown?'再给一点提示':'给我一点提示';}if(stepBtn&&stepIndex<steps.length){stepBtn.disabled=false;stepBtn.textContent=stepIndex?'看下一步':'看关键步骤';}if(answerBtn){answerBtn.disabled=false;answerBtn.textContent='查看答案与解析';}}catch(err){if(seq===request&&key===identityKey())status(message,'保存失败，答案仍保留在输入框。请重试。',true);}finally{button.disabled=false;}};
 });
