...
async function open(date){
 ...
 btn.onclick=()=>{if(shown>=hints.length)return;const p=document.createElement('div');p.className='daily-hint';p.textContent=`提示 ${shown+1}：${hints[shown++]}`;card.querySelector('.daily-hints').append(p);btn.textContent=shown===hints.length?'提示已全部展开':'再给一点提示';btn.disabled=shown===hints.length;window.QuestionContent?.math(p);};
 const stepBtn=card.querySelector('.daily-step-btn');let stepIndex=0;const steps=currentSet.questions[slot].steps||[];if(stepBtn)stepBtn.onclick=()=>{const p=document.createElement('div');p.className='daily-hint';p.textContent=`步骤 ${stepIndex+1}：${steps[stepIndex++]}`;card.querySelector('.daily-steps').append(p);window.QuestionContent?.math(p);stepBtn.disabled=stepIndex===steps.length;stepBtn.textContent=stepBtn.disabled?'步骤已全部展开':'看下一步';};
