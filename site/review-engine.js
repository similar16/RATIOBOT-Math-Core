...
function settle(p,run,answers,{today=day(),now=Date.now()}={}){
 ...
 if(mod.receipts.includes(run.id))return {duplicate:true};const result=grade(run.q,answers),r=c.days[today]||(c.days[today]={points:0,checked:false,full:false,runs:{}});const events=e.events||(e.events={}),festival=events.midAutumn2026||(events.midAutumn2026={days:{},unlocked:false,equipped:false});festival.days=festival.days||{};const festivalDay=festival.days[today]||(festival.days[today]={knowledgeBlanks:0,mixedRounds:0,supplement:false});festivalDay.knowledgeBlanks=Number(festivalDay.knowledgeBlanks||0)+run.q.blanks.length;
